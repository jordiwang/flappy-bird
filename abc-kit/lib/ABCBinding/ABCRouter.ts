 ...
 */
import ABCBinding from './ABCBinding';

const TAG = 'ABCRouter';
export default class ABCRouter {
    private static instance: ABCRouter;
    ...
    public registerMethod(eventName: string, callback: Function) {
        ...
        window[`abcbinding_router_${eventName}`] = (args) => {
    ...
    public unRegisterMethod(eventName: string) {
        ...
        delete window[`abcbinding_router_${eventName}`];
    }
            ...
            if (cc.sys.isNative && typeof exeResult === 'object') {
                if (exeResult instanceof Promise) {
                    exeResult.then((result) => {
                        ...
                        ABCBinding.withOptions({hasCallback: false}).callNativeMethod(callbackName, result)
                            .then((rs) => {
                                cc.log('receive:', rs);
                            }).catch(e => {
                            cc.log('exception:', e);
                    ...
                    cc.log('start call native from ts');
                    const callbackName = args['@nativeCallbackName'];
                    ABCBinding.withOptions({hasCallback: false}).callNativeMethod(callbackName, exeResult)
                        .then((rs) => {
                            cc.log('receive:', rs);
                        }).catch(e => {
                        cc.log('exception:', e);
